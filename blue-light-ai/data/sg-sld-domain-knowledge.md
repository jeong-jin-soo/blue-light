...
### 서브회로 ISOLATOR의 렌더링 규칙

**중요**: 서브회로의 ISOLATOR(ISOL1, ISOL2 등)는 스파인(메인 라인) 위의 ISOLATOR와 **다르게 렌더링**된다.

| 위치 | 렌더링 방식 |
|------|------------|
| 스파인 (Unit Isolator, Meter Board) | ISOLATOR 전용 심볼 (enclosed/open switch) |
| **서브회로** (ISOL1, ISOL2...) | **MCB 심볼** (반원 arc) + **도체선 끝에 DP_ISOL_DEVICE** (우측 꼭지 달린 사각형) |

- circuit_id에 "ISOL"이 포함되어 있어도 서브회로에서는 MCB 심볼로 그린다
- DP_ISOL_DEVICE가 도체선 상단 끝에 부착되어 ISOLATOR임을 표시한다
- 이것은 레퍼런스 DWG/PDF에서 일관되게 확인된 싱가포르 SLD 표준 패턴이다

### 비교 규칙

1. **존재 여부 먼저** — 각 섹션이 렌더링되었는지부터 확인 (없는 것은 비교 불가)
2. **심볼 형태** — 동일 기능이라도 enclosed/open, 원형/사각형 등 시각적 차이 확인
   - **⚠️ 추정 금지**: "이름이 ISOL이니까 ISOLATOR 심볼일 것이다" 같은 추정을 하지 말 것. 반드시 레퍼런스의 실제 렌더링 형상을 확인한 후에 비교할 것.
