...
| **D-9** | **입력 검증 오류는 잠금 미카운트** | 토큰 5회 실패 잠금(H-3)은 **토큰 자체 유효성 오류(만료/이미 사용/무효)만** `recordFailure` 카운트. 비번 정책·면허 중복·PayNow 형식 등 **입력 검증 오류는 미카운트** — 면허/PayNow 오타로 본인 토큰이 잠기는 사고 방지. (기존 `AccountSetupService.complete`와 일관.) |
