# LicenseKaki 싱가포르 웹 도메인 셋팅 가이드

## 1. 도메인 종류 비교

| 도메인 | 예시 | 연 비용 | 특징 |
|--------|------|---------|------|
| **.com.sg** | `licensekaki.com.sg` | **SGD 28~50/년** | 싱가포르 상업용. 사업자등록(ACRA) 필요 → 신뢰도 최고 |
| **.sg** | `licensekaki.sg` | SGD 28~50/년 | 싱가포르 일반용. 사업자 아니어도 가능, 짧고 깔끔 |
| **.com** | `licensekaki-sg.com` | **USD 7~15/년** | 제한 없음. 글로벌 인지도 높지만 싱가포르 SEO 불리 |

### 추천 전략

```
1순위:  licensekaki.com.sg  — 싱가포르 비즈니스 신뢰도 최고
2순위:  licensekaki.sg      — 짧고 모던, 동시 구매 권장
3순위:  licensekaki.com     — 글로벌 확장용 (이미 타인 소유 가능성 높음)
```
...
```
licensekaki.com.sg DNS Records (Cloudflare)

| Name     | Type  | Value                 | Proxy        |
|----------|-------|-----------------------|--------------|
| @        | A     | ALB IP (또는 CNAME)    | Proxied      |
| www      | CNAME | licensekaki.com.sg      | Proxied      |
...
| 서브도메인 | 용도 |
|-----------|------|
| `licensekaki.com.sg` | 프론트엔드 (React) |
| `api.licensekaki.com.sg` | 백엔드 API (Spring Boot) |
| `www.licensekaki.com.sg` | → 메인 도메인 리다이렉트 |
...
2. "Add a Site" → licensekaki.com.sg 입력
...
CORS_ALLOWED_ORIGINS=https://licensekaki.com.sg,https://www.licensekaki.com.sg

# Frontend .env
VITE_API_BASE_URL=https://api.licensekaki.com.sg
```
