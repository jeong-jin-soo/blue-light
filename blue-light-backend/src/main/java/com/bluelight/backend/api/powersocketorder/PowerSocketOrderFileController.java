...
/**
 * Power Socket 주문 파일 업로드 컨트롤러 (SldOrderFileController와 동일 구조).
 */
@Slf4j
@RestController
@RequiredArgsConstructor
public class PowerSocketOrderFileController {

    private final FileStorageService fileStorageService;
    private final FileRepository fileRepository;
    private final PowerSocketOrderRepository powerSocketOrderRepository;

    @PostMapping("/api/power-socket-orders/{orderId}/files")
    @Transactional
    public ResponseEntity<PowerSocketOrderFileResponse> uploadApplicantFile(
        ...
        log.info("Power Socket 파일 업로드: userSeq={}, orderId={}, fileType={}, filename={}",
                userSeq, orderId, fileType, file.getOriginalFilename());

        PowerSocketOrder order = findOrderOrThrow(orderId);
    ...
    @PostMapping("/api/power-socket-manager/orders/{orderId}/files")
    @PreAuthorize("hasAnyRole('SLD_MANAGER', 'ADMIN', 'SYSTEM_ADMIN')")
    @Transactional
    public ResponseEntity<PowerSocketOrderFileResponse> uploadManagerFile(
            @PathVariable Long orderId,
            @RequestParam("file") MultipartFile file,
            @RequestParam("fileType") String fileType) {

        log.info("PowerSocket Manager 파일 업로드: orderId={}, fileType={}, filename={}",
                orderId, fileType, file.getOriginalFilename());
        PowerSocketOrder order = findOrderOrThrow(orderId);
        return ResponseEntity.status(HttpStatus.CREATED).body(storeFile(order, file, fileType));
    }

    private PowerSocketOrderFileResponse storeFile(PowerSocketOrder order, MultipartFile file, String fileType) {
        validateFile(file);

        FileType parsedFileType = parseFileType(fileType);
        String subDirectory = "power-socket-orders/" + order.getPowerSocketOrderSeq();
        ...
        log.info("Power Socket 주문 파일 저장 완료: fileSeq={}, orderId={}, type={}",
                savedFile.getFileSeq(), order.getPowerSocketOrderSeq(), parsedFileType);

        return PowerSocketOrderFileResponse.from(savedFile);
    }
    ...
    private PowerSocketOrder findOrderOrThrow(Long orderId) {
        return powerSocketOrderRepository.findById(orderId)
                .orElseThrow(() -> new BusinessException(
                        "Power Socket order not found", HttpStatus.NOT_FOUND,
                        "POWERSOCKETORDER_NOT_FOUND"));
    }

    @Getter
    @Builder
    public static class PowerSocketOrderFileResponse {
        ...
        public static PowerSocketOrderFileResponse from(FileEntity entity) {
            return PowerSocketOrderFileResponse.builder()
