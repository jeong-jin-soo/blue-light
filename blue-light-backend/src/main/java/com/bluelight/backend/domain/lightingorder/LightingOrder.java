...
/**
 * Lighting Layout 주문 Entity
 * - 라이센스 신청 없이 SLD 도면만 요청하는 경우
 * - SLD_MANAGER가 견적 제안 → 신청자 수락 → 결제 → SLD 생성 → 완료
 */
@Entity
@Table(name = "lighting_orders")
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@SQLDelete(sql = "UPDATE lighting_orders SET deleted_at = NOW() WHERE lighting_order_seq = ?")
@SQLRestriction("deleted_at IS NULL")
public class LightingOrder extends BaseEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "lighting_order_seq")
    ...
    /**
     * 담당 Lighting Manager (FK, nullable)
     */
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    private LightingOrderStatus status = LightingOrderStatus.PENDING_QUOTE;
    ...
    /**
     * 완성된 Lighting Layout 파일 (FK → files)
     */
    ...
    @Builder
    public LightingOrder(User user, String address, String postalCode,
        ...
        this.status = LightingOrderStatus.PENDING_QUOTE;
    }
    ...
    public void acceptQuote() {
        if (this.status != LightingOrderStatus.QUOTE_PROPOSED) {
            throw new IllegalStateException("견적 수락은 QUOTE_PROPOSED 상태에서만 가능합니다. 현재: " + this.status);
        }
        this.status = LightingOrderStatus.PENDING_PAYMENT;
    }
    ...
    public void rejectQuote() {
        ...
        this.status = LightingOrderStatus.QUOTE_REJECTED;
    }
    ...
    public void markAsPaid() {
        if (this.status != LightingOrderStatus.PENDING_PAYMENT) {
            throw new IllegalStateException("결제 확인은 PENDING_PAYMENT 상태에서만 가능합니다. 현재: " + this.status);
        }
        this.status = LightingOrderStatus.PAID;
    }
    ...
    public void startWork() {
        if (this.status != LightingOrderStatus.PAID) {
            throw new IllegalStateException("작업 시작은 PAID 상태에서만 가능합니다. 현재: " + this.status);
        }
        this.status = LightingOrderStatus.IN_PROGRESS;
    }

    /**
     * Lighting Layout 업로드 완료 (SLD_MANAGER)
     */
    public void uploadSld(Long fileSeq, String managerNote) {
        if (this.status != LightingOrderStatus.IN_PROGRESS
                && this.status != LightingOrderStatus.REVISION_REQUESTED
                && this.status != LightingOrderStatus.SLD_UPLOADED) {
            throw new IllegalStateException("Lighting Layout 업로드는 IN_PROGRESS, REVISION_REQUESTED 또는 SLD_UPLOADED 상태에서만 가능합니다. 현재: " + this.status);
        }
    ...
    public void requestRevision(String comment) {
        if (this.status != LightingOrderStatus.SLD_UPLOADED) {
    ...
    public void complete() {
        ...
        this.status = LightingOrderStatus.COMPLETED;
    }
