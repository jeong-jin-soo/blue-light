...
/**
 * Power Socket 주문 Entity
 * - 라이센스 신청 없이 SLD 도면만 요청하는 경우
 * - SLD_MANAGER가 견적 제안 → 신청자 수락 → 결제 → SLD 생성 → 완료
 */
@Entity
@Table(name = "power_socket_orders")
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@SQLDelete(sql = "UPDATE power_socket_orders SET deleted_at = NOW() WHERE power_socket_order_seq = ?")
@SQLRestriction("deleted_at IS NULL")
public class PowerSocketOrder extends BaseEntity {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "power_socket_order_seq")
    ...
    /**
     * 담당 PowerSocket Manager (FK, nullable)
     */
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    private PowerSocketOrderStatus status = PowerSocketOrderStatus.PENDING_QUOTE;
    ...
    @Builder
    public PowerSocketOrder(User user, String address, String postalCode,
        ...
        this.status = PowerSocketOrderStatus.PENDING_QUOTE;
    }
    ...
    public void acceptQuote() {
        if (this.status != PowerSocketOrderStatus.QUOTE_PROPOSED) {
            throw new IllegalStateException("견적 수락은 QUOTE_PROPOSED 상태에서만 가능합니다. 현재: " + this.status);
        }
        this.status = PowerSocketOrderStatus.PENDING_PAYMENT;
    }
    ...
    public void rejectQuote() {
        ...
        this.status = PowerSocketOrderStatus.QUOTE_REJECTED;
    }
    ...
    public void markAsPaid() {
        if (this.status != PowerSocketOrderStatus.PENDING_PAYMENT) {
            throw new IllegalStateException("결제 확인은 PENDING_PAYMENT 상태에서만 가능합니다. 현재: " + this.status);
        }
        this.status = PowerSocketOrderStatus.PAID;
    }
    ...
    public void startWork() {
        if (this.status != PowerSocketOrderStatus.PAID) {
            throw new IllegalStateException("작업 시작은 PAID 상태에서만 가능합니다. 현재: " + this.status);
        }
        this.status = PowerSocketOrderStatus.IN_PROGRESS;
    }

    /**
     * Power Socket 업로드 완료 (SLD_MANAGER)
     */
    public void uploadSld(Long fileSeq, String managerNote) {
        if (this.status != PowerSocketOrderStatus.IN_PROGRESS
                && this.status != PowerSocketOrderStatus.REVISION_REQUESTED
                && this.status != PowerSocketOrderStatus.SLD_UPLOADED) {
            throw new IllegalStateException("Power Socket 업로드는 IN_PROGRESS, REVISION_REQUESTED 또는 SLD_UPLOADED 상태에서만 가능합니다. 현재: " + this.status);
        }
    ...
    public void requestRevision(String comment) {
        if (this.status != PowerSocketOrderStatus.SLD_UPLOADED) {
    ...
    public void complete() {
        ...
        this.status = PowerSocketOrderStatus.COMPLETED;
    }

    /**
     * PowerSocket Manager 배정
     */
