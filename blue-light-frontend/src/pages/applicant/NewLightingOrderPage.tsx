...
export default function NewLightingOrderPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (sketchFile) {
        try {
          await lightingOrderApi.uploadSketchFile(order.lightingOrderSeq, sketchFile, 'SKETCH_LIGHTING');
        } catch {
          toast.warning('Lighting Layout order created, but sketch upload failed. You can upload it later.');
