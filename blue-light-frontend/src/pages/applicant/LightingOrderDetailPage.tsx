...
import type { LightingOrder, LightingOrderStatus } from '../../types';

const STATUS_CONFIG: Record<LightingOrderStatus, { label: string; color: string }> = {
...
function LightingStatusBadge({ status }: { status: LightingOrderStatus }) {
...
export default function LightingOrderDetailPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const toast = useToastStore();

  const [order, setOrder] = useState<LightingOrder | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      const orderData = await lightingOrderApi.getLightingOrder(orderId);
      setOrder(orderData);
      ...
      toast.error('Failed to load Lighting Layout order details');
      navigate('/lighting-orders');
    } finally {
      setLoading(false);
    }
  }, [orderId]);
  ...
  const handleAcceptQuote = async () => {
    ...
    try {
      await lightingOrderApi.acceptQuote(orderId);
  ...
  const handleRejectQuote = async () => {
    ...
    setActionLoading(true);
    try {
      await lightingOrderApi.rejectQuote(orderId);
  ...
  const handleRequestRevision = async () => {
    ...
    setActionLoading(true);
    try {
      await lightingOrderApi.requestRevision(orderId, revisionComment.trim());
  ...
  const handleConfirmCompletion = async () => {
    ...
    setActionLoading(true);
    try {
      await lightingOrderApi.confirmCompletion(orderId);
      toast.success('Lighting Layout order completed!');
      fetchData();
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <LoadingSpinner size="lg" label="Loading Lighting Layout order..." />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <PageHeader
        title={`Lighting Layout Order #${order.lightingOrderSeq}`}
        subtitle={`Requested on ${new Date(order.createdAt).toLocaleDateString()}`}
        actions={<LightingStatusBadge status={order.status} />}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-700 mt-1">
                      Payment confirmed. Lighting work will begin shortly.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <Card>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <span className="text-lg">&#128736;</span>
                  <div>
                    <p className="text-sm font-medium text-blue-800">Lighting Layout In Progress</p>
                    <p className="text-xs text-blue-700 mt-1">
                      Your Lighting work is being prepared. You will be notified once it is uploaded.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {order.status === 'SLD_UPLOADED' && (
            <Card>
              <h2 className="text-lg font-semibold text-gray-800 mb-4">Review Lighting Layout</h2>
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 space-y-3">
                <div className="flex items-start gap-3">
                  <span className="text-lg">&#128196;</span>
                  <div>
                    <p className="text-sm font-medium text-purple-800">Lighting deliverable has been uploaded</p>
                ...
                {order.uploadedFileSeq && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleDownloadFile(order.uploadedFileSeq!, 'Lighting_Deliverable')}
                  >
                    Download Lighting
                  </Button>
                )}
              ...
              {pdfPreviewUrl && (
                <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden">
                  <iframe
                    src={pdfPreviewUrl}
                    title="Lighting Deliverable Preview"
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-green-700 mt-1">
                      Your Lighting Layout order has been completed.
                    </p>
                    {order.uploadedFileSeq && (
                      <Button
                        variant="outline"
          ...
          <Card>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-700">#{order.lightingOrderSeq}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Status</span>
                <LightingStatusBadge status={order.status} />
              </div>
      ...
      <ConfirmDialog
        ...
        message="Confirm that the uploaded Lighting Layout is complete and acceptable? This action cannot be undone. Once confirmed, no further revisions can be requested."
