...
export default function PowerSocketOrderDetailPage() {
  const { id } = useParams();
  ...
  const fetchData = useCallback(async () => {
    try {
      const orderData = await powerSocketOrderApi.getPowerSocketOrder(orderId);
      setOrder(orderData);
  ...
  const handleAcceptQuote = async () => {
    ...
    try {
      await powerSocketOrderApi.acceptQuote(orderId);
  ...
  const handleRejectQuote = async () => {
    ...
    setActionLoading(true);
    try {
      await powerSocketOrderApi.rejectQuote(orderId);
  ...
  const handleRequestRevision = async () => {
    ...
    setActionLoading(true);
    try {
      await powerSocketOrderApi.requestRevision(orderId, revisionComment.trim());
  ...
  const handleConfirmCompletion = async () => {
    ...
    setActionLoading(true);
    try {
      await powerSocketOrderApi.confirmCompletion(orderId);
      toast.success('Power Socket order completed!');
      fetchData();
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <PageHeader
        title={`Power Socket Order #${order.powerSocketOrderSeq}`}
        subtitle={`Requested on ${new Date(order.createdAt).toLocaleDateString()}`}
        actions={<PowerSocketStatusBadge status={order.status} />}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-700 mt-1">
                      Payment confirmed. Power Socket work will begin shortly.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <Card>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <span className="text-lg">&#128736;</span>
                  <div>
                    <p className="text-sm font-medium text-blue-800">Power Socket In Progress</p>
                    <p className="text-xs text-blue-700 mt-1">
                      Your Power Socket work is being prepared. You will be notified once it is uploaded.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {order.status === 'SLD_UPLOADED' && (
            <Card>
              <h2 className="text-lg font-semibold text-gray-800 mb-4">Review Power Socket</h2>
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 space-y-3">
                <div className="flex items-start gap-3">
                  <span className="text-lg">&#128196;</span>
                  <div>
                    <p className="text-sm font-medium text-purple-800">Power Socket deliverable has been uploaded</p>
                ...
                {order.uploadedFileSeq && (
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleDownloadFile(order.uploadedFileSeq!, 'PowerSocket_Deliverable')}
                  >
                    Download Power Socket
                  </Button>
                )}
              </div>
              ...
              {pdfPreviewUrl && (
                <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden">
                  <iframe
                    src={pdfPreviewUrl}
                    title="Power Socket Deliverable Preview"
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-green-700 mt-1">
                      Your Power Socket order has been completed.
                    </p>
                    {order.uploadedFileSeq && (
                      <Button
                        variant="outline"
      ...
      <ConfirmDialog
        ...
        message="Confirm that the uploaded Power Socket is complete and acceptable? This action cannot be undone. Once confirmed, no further revisions can be requested."
