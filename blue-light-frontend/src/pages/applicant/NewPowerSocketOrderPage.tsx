...
export default function NewPowerSocketOrderPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const order = await powerSocketOrderApi.createPowerSocketOrder(payload);

      if (sketchFile) {
        try {
          await powerSocketOrderApi.uploadSketchFile(order.powerSocketOrderSeq, sketchFile, 'SKETCH_POWER_SOCKET');
        } catch {
          toast.warning('Power Socket order created, but sketch upload failed. You can upload it later.');
          navigate(`/power-socket-orders/${order.powerSocketOrderSeq}`);
          return;
        }
      }

      toast.success('Power Socket order submitted successfully!');
      navigate(`/power-socket-orders/${order.powerSocketOrderSeq}`);
    } catch {
      toast.error('Failed to submit Power Socket order. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800">New Power Socket Order</h1>
          <p className="text-sm text-gray-500 mt-0.5">Request a power socket layout drawing</p>
        </div>
      </div>
        ...
        <Card>
          <div className="space-y-5">
            ...
            <Textarea
              ...
              placeholder="Describe your requirements for the Power Socket..."
