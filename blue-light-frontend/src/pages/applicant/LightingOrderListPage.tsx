...
export default function LightingOrderListPage() {
  const navigate = useNavigate();
  const toast = useToastStore();
  const [orders, setOrders] = useState<LightingOrder[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    lightingOrderApi.getMyLightingOrders()
      .then(setOrders)
      .catch(() => toast.error('Failed to load Lighting Layout orders'))
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <LoadingSpinner size="lg" label="Loading Lighting Layout orders..." />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800">My Lighting Layout Orders</h1>
          <p className="text-sm text-gray-500 mt-1">Your Lighting Layout order history</p>
        </div>
        <Button onClick={() => navigate('/lighting-orders/new')}>
          + New Lighting Layout Order
        </Button>
      </div>
        ...
        {orders.length === 0 ? (
          ...
          <>
            ...
            <div className="sm:hidden divide-y divide-gray-100">
              {orders.map((order) => (
                <div
                  key={order.lightingOrderSeq}
                  className="py-3 cursor-pointer active:bg-gray-50"
                  role="button"
                  tabIndex={0}
                  onClick={() => navigate(`/lighting-orders/${order.lightingOrderSeq}`)}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/lighting-orders/${order.lightingOrderSeq}`); } }}
                >
                  <div className="flex items-start justify-between mb-1.5">
                    <div className="min-w-0 flex-1 mr-3">
                      <p className="font-medium text-gray-800 truncate">
                        {order.address || `Lighting Layout Order #${order.lightingOrderSeq}`}
                      </p>
                  ...
                  {orders.map((order) => (
                    ...
                    >
                      <td className="py-3 px-2">
                        <span className="font-mono text-xs text-gray-500">#{order.lightingOrderSeq}</span>
                      </td>
                      <td className="py-3 px-2">
