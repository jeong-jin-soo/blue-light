...
export default function PowerSocketManagerOrderListPage() {
  ...
  useEffect(() => {
    ...
    powerSocketManagerApi
      ...
      .catch((err: { message?: string }) => {
        toast.error(err.message || 'Failed to load Power Socket orders');
      })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Power Socket Orders</h1>
        <p className="text-sm text-gray-500 mt-1">Manage all Power Socket orders</p>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            icon="🔌"
            title="No Power Socket orders found"
            description={
              statusFilter
                ? 'No orders match your filter criteria.'
                : 'Power Socket orders will appear here once applicants submit them.'
            }
          />
        </Card>
      ) : (
          ...
          <div className="sm:hidden space-y-2">
            {orders.map((order) => (
              <Card
                ...
                role="button"
                tabIndex={0}
                onClick={() => navigate(`/power-socket-manager/orders/${order.powerSocketOrderSeq}`)}
                onKeyDown={(e: React.KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/power-socket-manager/orders/${order.powerSocketOrderSeq}`); } }}
              >
                ...
                <tbody>
                  {orders.map((order) => (
                    <tr
                      ...
                      tabIndex={0}
                      onClick={() => navigate(`/power-socket-manager/orders/${order.powerSocketOrderSeq}`)}
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/power-socket-manager/orders/${order.powerSocketOrderSeq}`); } }}
                    >
