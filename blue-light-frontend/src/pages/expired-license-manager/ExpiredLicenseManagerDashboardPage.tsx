...
import {
  Search, MessageSquare, CreditCard, BadgeCheck, CalendarClock,
  FileText, RotateCcw, Flag, ClipboardList, ChevronRight,
} from 'lucide-react';
...
export default function ExpiredLicenseManagerDashboardPage() {
  ...
  const chips = [
    ...
    { label: 'Visit Scheduled', value: dashboard?.visitScheduled ?? 0, icon: CalendarClock, q: 'VISIT_SCHEDULED' },
    { label: 'Report Submitted', value: dashboard?.visitCompleted ?? 0, icon: FileText, q: 'VISIT_COMPLETED' },
    { label: 'Revisit Requested', value: dashboard?.revisitRequested ?? 0, icon: RotateCcw, q: 'REVISIT_REQUESTED' },
    { label: 'Completed', value: dashboard?.completed ?? 0, icon: Flag, q: 'COMPLETED' },
  ];

  const goOrders = (q: string) =>
    navigate(q ? `/expired-license-manager/orders?status=${q}` : '/expired-license-manager/orders');

  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <PageHeader title="Expired License Manager Dashboard" subtitle="Overview of Expired License renewal orders" />
