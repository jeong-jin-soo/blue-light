...
export function VisitCompletionForm({ orderId, order, onCompleted }: Props) {
  ...
  const uploadPhotos = async () => {
    ...
    try {
      await expiredLicenseManagerApi.uploadVisitPhotos(orderId, photos, captions);
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      const uploaded = await expiredLicenseManagerApi.uploadFile(
        orderId,
        reportFile,
        'LEW_SERVICE_VISIT_REPORT',
      );
      // 3) Check-out
      await expiredLicenseManagerApi.checkOut(orderId, {
