...
export default function LightingManagerOrderListPage() {
  ...
  useEffect(() => {
    ...
    lightingManagerApi
      ...
      .catch((err: { message?: string }) => {
        toast.error(err.message || 'Failed to load Lighting Layout orders');
      })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Lighting Layout Orders</h1>
        <p className="text-sm text-gray-500 mt-1">Manage all Lighting Layout orders</p>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState
            icon="💡"
            title="No Lighting Layout orders found"
            description={
              statusFilter
                ? 'No orders match your filter criteria.'
                : 'Lighting Layout orders will appear here once applicants submit them.'
            }
          />
        </Card>
      ) : (
          ...
          <div className="sm:hidden space-y-2">
            {orders.map((order) => (
              <Card
                ...
                role="button"
                tabIndex={0}
                onClick={() => navigate(`/lighting-manager/orders/${order.lightingOrderSeq}`)}
                onKeyDown={(e: React.KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/lighting-manager/orders/${order.lightingOrderSeq}`); } }}
              >
                ...
                <tbody>
                  {orders.map((order) => (
                    <tr
                      ...
                      tabIndex={0}
                      onClick={() => navigate(`/lighting-manager/orders/${order.lightingOrderSeq}`)}
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`/lighting-manager/orders/${order.lightingOrderSeq}`); } }}
                    >
