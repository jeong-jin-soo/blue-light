...
import { LightingManagerSection } from './sections/LightingManagerSection';
import type { LightingOrder } from '../../types';

export default function LightingManagerOrderDetailPage() {
  const { id } = useParams();
  ...
  const fetchData = useCallback(async () => {
    try {
      const data = await lightingManagerApi.getOrder(orderId);
      setOrder(data);
    } catch {
      toast.error('Failed to load Lighting Layout order details');
      navigate('/lighting-manager/orders');
    } finally {
      setLoading(false);
    }
  }, [orderId]);
  ...
  const handleProposeQuote = async () => {
    ...
    try {
      await lightingManagerApi.proposeQuote(orderId, {
        quoteAmount: amount,
  ...
  const handleConfirmPayment = async () => {
    ...
    setActionLoading(true);
    try {
      await lightingManagerApi.confirmPayment(orderId);
  ...
  const handleDeliverableUpload = async (file: File, managerNote?: string) => {
    const uploadedFile = await lightingManagerApi.uploadFile(orderId, file, 'DRAWING_SLD');
    await lightingManagerApi.uploadDeliverableComplete(orderId, uploadedFile.fileSeq, managerNote);
    toast.success('Lighting Layout uploaded and marked as complete');
    fetchData();
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <span className="text-lg">&#9989;</span>
                  <div>
                    <p className="text-sm font-medium text-green-800">Payment confirmed. Begin work on the Lighting Layout.</p>
                    <p className="text-xs text-green-700 mt-1">
                      Payment has been confirmed. You can now prepare and upload the Lighting Layout.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <Card>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  <span className="text-lg">&#128736;</span>
                  <div>
                    <p className="text-sm font-medium text-blue-800">Lighting Layout Work In Progress</p>
                    <p className="text-xs text-blue-700 mt-1">
                      Use the section below to upload the Lighting Layout.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {showDeliverableSection && (
            <LightingManagerSection onDeliverableUpload={handleDeliverableUpload} />
          )}
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-purple-800">Lighting Layout uploaded. Waiting for applicant review.</p>
                    ...
                    {order.uploadedFileSeq && (
                      <Button
                        ...
                        size="sm"
                        className="mt-2"
                        onClick={() => handleDownloadFile(order.uploadedFileSeq!, 'Lighting_Layout')}
                      >
                        Download Lighting Layout
                      </Button>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-green-700 mt-1">
                      This Lighting Layout order has been completed successfully.
                    </p>
                    {order.uploadedFileSeq && (
                      <Button
                        variant="outline"
