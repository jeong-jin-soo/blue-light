...
export default function SldManagerDashboardPage() {
  ...
  const chips = [
    ...
    { label: 'Uploaded', value: dashboard?.sldUploaded ?? 0, icon: FileText, q: 'SLD_UPLOADED' },
    { label: 'Completed', value: dashboard?.completed ?? 0, icon: Flag, q: 'COMPLETED' },
  ];

  const goOrders = (q: string) =>
    navigate(q ? `/sld-manager/orders?status=${q}` : '/sld-manager/orders');

  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <PageHeader title="SLD Manager Dashboard" subtitle="Overview of SLD drawing orders" />
