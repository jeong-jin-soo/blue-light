...
export default function LewServiceManagerOrderDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const data = await lewServiceManagerApi.getOrder(orderId);
      setOrder(data);
    } catch {
      toast.error('Failed to load LEW Service order details');
      navigate('/lew-service-manager/orders');
    } finally {
      setLoading(false);
    }
  }, [orderId]);
  ...
  const handleProposeQuote = async () => {
    ...
    try {
      await lewServiceManagerApi.proposeQuote(orderId, {
        quoteAmount: amount,
  ...
  const handleConfirmPayment = async () => {
    ...
    setActionLoading(true);
    try {
      await lewServiceManagerApi.confirmPayment(orderId);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* Page header */}
      <PageHeader
        title={`LEW Service Order #${order.lewServiceOrderSeq}`}
        subtitle="Manager view"
        actions={<LewServiceStatusBadge status={order.status} />}
      />
