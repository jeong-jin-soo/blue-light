 ...
 *
 * 서버 가드(LewReviewService.requestPayment): kVA CONFIRMED + 미해결 서류 0건.
...
export interface UseRequestPaymentOptions {
  ...
  /**
   * 가드 위반(KVA_NOT_CONFIRMED / DOCUMENT_REQUESTS_PENDING)으로 리뷰가 필요할 때 호출.
   * - 진입 페이지: 리뷰 폼으로 navigate.
   * - 리뷰 폼: 이미 그 화면이므로 해당 탭으로 점프(또는 no-op).
   * 미지정 시 기본 동작은 리뷰 URL로 navigate.
   */
  onNeedsReview?: (reason: 'kva' | 'documents') => void;
